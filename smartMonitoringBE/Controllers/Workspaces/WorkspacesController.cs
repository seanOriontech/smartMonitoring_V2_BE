...
public sealed class WorkspacesController : ControllerBase
{
    ...
    [HttpGet("{workspaceId:guid}", Name = "GetWorkspace")]
    [Authorize(Policy = "WorkspaceAccess")]
    public async Task<IActionResult> GetOne(Guid accountId, Guid workspaceId, CancellationToken ct)
        => Ok(await _svcr.GetWorkspaceTreeAsync(accountId, ct));

  /*  [HttpDelete("{workspaceId:guid}")]
    [Authorize(Policy = "WorkspaceAccess")]
    public async Task<IActionResult> Archive([FromRoute] Guid accountId, [FromRoute] Guid workspaceId, CancellationToken ct)
    {
        await _svc.ArchiveWorkspaceAsync(accountId, workspaceId, ct);
        return NoContent();
    }*/
}